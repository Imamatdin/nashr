...
// Ported from beautifului.dev "context-cards" (MIT). Kept: the header count badge, the per-card
// fade-up stagger, the 700ms chipsShown delay with its per-card 80ms transition-delay and
// scale(0.95)→scale(1), the card bar / body / source-chip structure and the external-link glyph.
// Replaced: the CHUNKS fixture -> `chunks` prop; the tone is a named token role, not a raw class.
...
const TONE_CLASS: Record<ChunkTone, string> = {
  red: "bg-red",
  green: "bg-green",
  accent: "bg-accent",
  orange: "bg-orange",
...
export function ContextCards({ title, count, chunks }: ContextCardsProps) {
      ...
      {chunks.map((chunk, i) => (
        ...
        >
          ...
          <div className="px-3 pb-3">
            ...
            >
              <span
                className={`flex size-3.5 items-center justify-center rounded-[4px] ${TONE_CLASS[chunk.tone]} text-[7px] font-bold text-on-accent`}
              >
                {chunk.badge}
              </span>
              {chunk.source}
